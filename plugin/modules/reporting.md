## Recommending & reporting — evidence + conventions

- Hedging options beside a strong recommendation ("accept with
  rationale", "defer / watch", "leave as-is") are noise — an option
  ...
  third exit, and where the call is genuinely the operator's the
  recommendation attached is the build. The reach extends past the
  deferral to its POLITE TWIN, which escapes because it presents as
  offering rather than withholding: the finding is named, the
  design stated, the recommendation given — and then permission is
  asked for an act the rules already settle. Booking is the
  standing case and never the operator's call: the ask costs the
...
- Convention: a decision only the operator can settle — intent,
  ...
  Its mechanism, stated because a principle reused without one
  ...
  by its own units (Calibration, priced units).
- A RULING — a decision issued by a party that does not execute
  ...
  a decision's voice; Model routing, a figure a judgment desk
  RELAYS is marked machine-computed or estimate).
  Convention: where a ruling depends on state it is
...
- Writing is not delivering — the channel decides what arrives, and
  ...
  the operator must read before approving is its own turn, the
  gated call the next. A question arriving mid-turn is answered
  only by its own labeled item leading the turn's final message —
  convention: the label opens `Re: <topic>`, the form the mid-turn
  guard reads. "Already answered" is a delivery claim the
  transcript cannot settle: the transcript proves writing, not
...
- An appended correction leaves BOTH versions standing — a second
  ...
  position, the first being what they reach. Scope is the CLAIM,
  not its carrier: a closed work item still MOVES to its closure
  home (Per-project accretion), where the move leaves nothing
  behind that reads as current. Appending FEELS like the honest
  form because it preserves the record, and that is the costume —
  an in-place edit preserves it too, the forge marking the
  artifact edited and version control keeping what it replaced.
  Two cases make appending right: the original cannot be edited,
  or someone has already acted on it and the correction's own
  SEQUENCE is what they need to see. Convention: a correction to a
  delivered artifact lands in it.
- Convention: operator-facing output — a conversational answer as
  ...
  entry point (a report re-entered from the bottom) may carry the
  lead's verdict again as ONE line opening its close — a pointer
  back, never a second summary body, so there is still exactly
  one summary to drift. The live state and
  open decisions above (LIVE
  state leads) appear in the lead first: it is the register those
  leading facts are written in, not a competing order. Evidence
  prose is written for the next session that must verify or resume
  the work — bases, pointers, terms of art, each correct by
  Grounding's conventions — and a report composed only in that
  register makes the operator route their own project through a
  translator (the relay shape). The
  lead is a doorway over its own body in the same message, never a
  persisted label, and it makes no claim the body does not ground —
  so it is not the paraphrase-drift class (Grounding). Compose-time
  tell: a first sentence carrying a token the operator never types —
  an internal symbol, an index into a structure they have never
  opened — is body, not lead. Altitude, the plain-language
  standard's mechanism: meaning travels in role-names, verification
  travels in attached identifiers. A body sentence's SUBJECT is the
...
- The report BODY is laid out for scanning, not only worded for it:
  ...
  woven into prose; a paragraph short enough that the reader can
  find the one they need without reading the others. The tell is
  the wall paragraph: a block holding three findings hides two.
- AI-authored content published under the operator's accounts —
  commits, PR bodies, issue/PR comments, posts, and any other
  channel that names an author — carries explicit AI attribution,
  never dropped silently: commits the Co-Authored-By trailer
